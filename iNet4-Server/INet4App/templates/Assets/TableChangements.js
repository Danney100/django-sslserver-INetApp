...
function UpdateJobsContent(result){
        ...
        for(var k in obj)
        {
            ...
            content += '<td class="Jnom">'+ obj[k]["JobNumber"] + '</td>';
            content += '<td class="JStatus">'+ FileStatusName(obj[k]["FileStatus"]) + '</td>';
            content += '<td class="JAuth">'+ obj[k]["AuthorNameLast"] + '</td>';
            content += '<td class="JTyp">'+ obj[k]["Secretary"] + '</td>';
            content += '<td class="JSub">'+ obj[k]["Subject"] + '</td>';
            content += '<td class="JWT">'+ obj[k]["Worktype"] + '</td>';
            content += '<td>'+ obj[k]["Length"] + '</td>';
            content += '<td class="JDate">'+ obj[k]["DateDictation"] + '</td>';
            content += '<td class="JTime">'+ obj[k]["TimeDictation"] + '</td>';
            content += '<td class="JDept">'+ obj[k]["Department"] + '</td>';
            content += '<td>'+ obj[k]["TransNameLast"] + '</td>';
            content += '<td>'+ obj[k]["DateTranscription"] + '</td>';
            content += '<td>'+ obj[k]["TimeTranscription"] + '</td>';
            content += '<td class="Fname" hidden>'+ obj[k]["subFname"]+'</td>';
            content += '<td class="JPri">'+ obj[k]["Priority"] + '</td>';
            content += '<td>'+ obj[k]["SystemId"] + '</td>';
            content += '<td>'+ obj[k]["Location"] + '</td>';
            content += '<td class="DTT" hidden>'+ obj[k]["DragonTranscribedText"] + '</td>';
            content += '<td class="JMarked">'+ obj[k]["Marked"] + '</td>';
        }
        ...
        $.ajax({
          ...
          success: function(result){
              ...
              document.getElementById('job_detail').innerText = "";
				let source = document.getElementById('audSRC');
				let audio = document.getElementById('AudioBLC');
				source.src = '';
				audio.load();
				audio.currentTime = 0
...
$(document).on("click", "tbody tr", function() {
            ...
            {
                //var snd = new Audio(BadBeepSound).play();
                alert("Please return your existing job before requesting another");
                return;
            }
            //Marked
            var JMarked = $(this).closest('tr').find('.JMarked').text();
            console.log(JMarked)
            if (JMarked != "0")
            {
                //new Audio(BadBeepSound).play();
                alert("This job is currently Marked and can not be signed out at this time");
                return;
            }
            ...
            var JStatus = $(this).closest('tr').find('.JStatus').text();


            // console.log(JMarked)
            // if (JStatus == "Saved")
            // {
            //     alert("This job is currently Saved (and not by you) and can not be signed out at this time");
            //     return;
            // }

            console.log("JStatus: " + JStatus)
            console.log("JSub1: " + JSub)
          ...
          $.ajax({
              ...
              success: function(result){
                ...
                console.log(r1)
                if (r1.includes("File_Already_Taken")){
                    //new Audio(BadBeepSound).play();
                    alert("Access is denied to this job. Someone may have beat you to it. Please refresh your list");
                    return;
                }
                ...
                // document.getElementById('JobCompleted').innerText = JStatus;
                //   console.log(JStatus);
...
// function ProcessRTFData(inData){
